...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateChart = (signalData) => {
        ...
        ecgChart = new Chart(chartCanvas, {
            ...
            options: {
                ...
                scales: {
                    y: { beginAtZero: false, title: { display: true, text: 'Amplitudo' } },
    ...
    const uploadAndDiagnose = async () => {
        ...
        try {
            ...
            const response = await fetch('http://localhost:5000/diagnose_ecg_signals', {
                method: 'POST',
                body: formData
            });

            if (!response.ok) {
                const errorData = await response.json().catch(() => ({ error: `Terjadi error di server: ${response.statusText}` }));
    ...
    // ==================================================================
    processButton.addEventListener('click', uploadAndDiagnose);
});
